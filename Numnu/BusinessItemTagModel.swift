...
struct BusinessItemTagModel {
    ...
    var tagItemList    : [BusinessItemtag]?
    ...
    init?(json : JSON){
        ...
        if let business = json["data"].array {
            
            for item in business {
                
                if let tagItem = BusinessItemtag(json: item) {
